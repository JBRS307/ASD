...
skiplist merge(skiplist A, skiplist B) {
	if (A.first->next[0]->val == max_int) return B;
	if (B.first->next[0]->val == max_int) return A;		// warunki konca moze kiedys przyda sie do merge sorta
	...
	while (A.first->next[0] != NULL && B.first->next[0] != NULL){ // na koncu jest + nsk wybierajac zawsze mniejsza badz rowna wartosc wkoncu przepniemy jedna nieskonczonosc i zostanie nam NULL
		if (A.first->next[0]->val < B.first->next[0]->val) {		// wybieram mniejsza wartosc
			for (int i = F.max_level - 1; i >= 0; i--) {
				if (A.first->next[0]->lvl > i){					// i tutaj lece od góry i jesli nalezy przepi¹æ dany poziom aktualizuje wskazniczek w tablicy na konkretnym poziomie
					tmp2 = A.first->next[0];
